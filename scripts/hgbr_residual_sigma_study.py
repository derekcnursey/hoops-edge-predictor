#!/usr/bin/env python3
"""Benchmark a first HGBR-native residual-scale sigma path with fixed HGBR mu."""
...
def _fit_residual_model(train_df: pd.DataFrame) -> tuple[HistGradientBoostingRegressor, np.ndarray]:
    ...
    y = train_df["abs_residual"].to_numpy(dtype=np.float32)
    model = HistGradientBoostingRegressor(**RESIDUAL_HGBR_PARAMS)
    model.fit(X, y)
    return model, means


def _predict_abs_residual(model: HistGradientBoostingRegressor, means: np.ndarray, df: pd.DataFrame) -> np.ndarray:
    X_raw = _prepare_feature_matrix(df)
    X = _apply_impute_means(X_raw, means)
    pred = model.predict(X).astype(np.float32)
    return np.clip(pred, 0.5, 30.0)


def _fit_scalar_from_predictions(df: pd.DataFrame, pred_abs_residual: np.ndarray) -> float:
    ...
    for c in np.arange(0.50, 2.51, 0.02):
        sigma = np.clip(c * pred_abs_residual, 0.5, 30.0)
...
def _crossval_scalar_fit(train_df: pd.DataFrame) -> tuple[float, str]:
    seasons = sorted(train_df["season"].unique().tolist())
    if len(seasons) == 1:
        model, means = _fit_residual_model(train_df)
        pred_abs = _predict_abs_residual(model, means, train_df)
        return _fit_scalar_from_predictions(train_df, pred_abs), "in_sample_single_season"
    ...
    for holdout in seasons:
        ...
        pred_df["pred_abs_residual_model"] = _predict_abs_residual(model, means, pred_df)
        oof_parts.append(pred_df)
    oof_df = pd.concat(oof_parts, ignore_index=True)
    return _fit_scalar_from_predictions(oof_df, oof_df["pred_abs_residual_model"].to_numpy(dtype=float)), "season_oof"
...
@click.option(
    "--output-dir",
    default="artifacts/hgbr_residual_sigma_study_v1",
    type=click.Path(path_type=Path),
    help="Output directory for the residual sigma study.",
)
def main(benchmark_dir: Path, output_dir: Path) -> None:
    ...
    for season in eval_seasons:
        ...
        scalar_c, scalar_fit_mode = _crossval_scalar_fit(train_df)
        resid_model, resid_means = _fit_residual_model(train_df)
        eval_pred_abs = _predict_abs_residual(resid_model, resid_means, eval_df)
        eval_sigma_resid = np.clip(scalar_c * eval_pred_abs, 0.5, 30.0)
        scalar_rows.append(
            {
                "season": season,
                "scalar_c": scalar_c,
                "scalar_fit_mode": scalar_fit_mode,
        ...
        option_sigmas = {
            ...
            "hgbr_residual_sigma": eval_sigma_resid,
        }
        ...
        for option, sigma in option_sigmas.items():
            ...
            if option == "hgbr_residual_sigma":
                pred_df["pred_abs_residual_model"] = eval_pred_abs
            ...
            fold_rows.append(
                {
                    "season": season,
                    "option": option,
                    "scalar_c": scalar_c if option == "hgbr_residual_sigma" else np.nan,
                    "scalar_fit_mode": scalar_fit_mode if option == "hgbr_residual_sigma" else "",
    ...
    scalar_df.to_csv(output_dir / "selected_scalars.csv", index=False)
    ...
    protocol = {
        ...
        "residual_target": "abs(actual_margin - mu_hgbr_oof)",
        "residual_model": {
            "family": "HistGradientBoostingRegressor",
            "params": RESIDUAL_HGBR_PARAMS,
        },
        "scalar_calibration": {
            "fit_objective": "cover_event_logloss",
            "grid": {"min": 0.50, "max": 2.50, "step": 0.02},
    ...
    click.echo(f"Residual sigma study written to {output_dir}")
