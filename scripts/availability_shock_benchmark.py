#!/usr/bin/env python3
"""Research-only benchmark for direct availability-shock features."""
...
VARIANTS = [
    ...
    Variant("torvik_availability_shock_v1", TORVIK, True),
    Variant("gold_priorreg_k5_v1", GOLD_PRIORREG, False),
    Variant("gold_priorreg_k5_v1_availability_shock_v1", GOLD_PRIORREG, True),
]


def _default_output_dir() -> Path:
    return config.ARTIFACTS_DIR / "lineup_research" / "availability_shock_benchmark_v1"
...
def _metrics(df: pd.DataFrame) -> dict[str, float]:
    ...
    return {"games": float(len(df)), "lined_games": float(lined.sum()), "mae_all": mae_all, "mae_lined": mae_lined}
...
def _season_features(source: EfficiencySource, season: int, shock_features: pd.DataFrame) -> pd.DataFrame:
    df = _prepare_frame(_load_or_build_features(source, season))
    if df.empty:
        return df
    return merge_availability_shock_features(df, shock_features)


def _fit_predict(variant: Variant, holdout: int, shock_features: pd.DataFrame) -> pd.DataFrame:
    train_frames = []
    for season in _training_seasons(holdout):
        train_frames.append(_season_features(variant.source, season, shock_features))
    holdout_df = _season_features(variant.source, holdout, shock_features)
    if not train_frames or holdout_df.empty:
        return pd.DataFrame()

    train_df = pd.concat(train_frames, ignore_index=True)
    feature_cols = list(config.FEATURE_ORDER)
    if variant.add_availability_shock:
        feature_cols = feature_cols + AVAILABILITY_SHOCK_FEATURE_COLUMNS
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark direct availability-shock features with fixed HGBR stack.")
    ...
    protocol = {
        ...
        "variants": [asdict(v.source) | {"variant_label": v.label, "add_availability_shock": v.add_availability_shock} for v in VARIANTS],
        "spine_usable": usable,
        "spine_reason": reason,
    }
    ...
    shock_features = build_availability_shock_team_features(flat_df, output_dir=spine_dir)
    ...
    for variant in VARIANTS:
        ...
        for holdout in holdouts:
            ...
            pred_df = _fit_predict(variant, holdout, shock_features)
            if pred_df.empty:
                continue
            ...
            for slice_name, slice_df in pred_df.groupby("slice"):
                slice_rows.append(
                    {"variant": variant.label, "holdout_season": holdout, "slice": slice_name, **_metrics(slice_df)}
                )
