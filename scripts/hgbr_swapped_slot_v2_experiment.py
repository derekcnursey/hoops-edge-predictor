#!/usr/bin/env python3
"""Research experiment: full swapped-slot augmentation on swap_safe_v2."""
...
MODEL_AUGMENTED_V2 = "HistGradientBoostingSwapSafeV2Augmented"
MIDDEC_CUTOFF = (12, 15)
FEATURE_CACHE_DIR = config.FEATURES_DIR / "contract_research"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run full swapped-slot augmentation on swap_safe_v2.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=config.ARTIFACTS_DIR / "research" / "hgbr_swapped_slot_v2_experiment_v1",
...
def _train_model(
    ...
    *,
    augment_swapped_slots: bool,
    full_swap_safe: bool = False,
    ...
    if augment_swapped_slots:
        feature_df, spread, _ = augment_swapped_slot_training(
            feature_df,
            spread,
            neutral_only=not full_swap_safe,
        )
...
def _predict_swapped_from_feature_df(
    ...
    *,
    ...
    swapped_df = swap_feature_frame(feature_df, feature_order, neutral_only=neutral_only)
...
def _predict_symmetrized_margin(
    ...
    if neutral_mask.any():
        ...
        pred_swap = _predict_swapped_from_feature_df(
            model,
            means,
            feature_df,
            feature_order,
...
def run_experiment(output_dir: Path) -> None:
    ...
    protocol = {
        "experiment": "hgbr_swapped_slot_v2_experiment_v1",
        ...
        "feature_contracts": {
            ...
            MODEL_AUGMENTED_V2: "swap_safe_v2 + full swapped-slot augmentation",
        },
    ...
    model_specs = [
        ...
        (MODEL_AUGMENTED_V2, list(config.FEATURE_ORDER_SWAP_SAFE_V2), _load_or_build_v2_features, True),
    ]
    ...
    for holdout_season in cw.HOLDOUT_SEASONS:
        ...
        for model_name, feature_order, _, augment in model_specs:
            ...
            model, means = _train_model(
                train_df,
                feature_order,
                augment_swapped_slots=augment,
                full_swap_safe=(feature_order == list(config.FEATURE_ORDER_SWAP_SAFE_V2)),
            )
            ...
            if neutral_mask.any():
                ...
                raw_pred_swap = _predict_swapped_from_feature_df(
                    ...
                    feature_order,
                    neutral_only=False if feature_order == list(config.FEATURE_ORDER_SWAP_SAFE_V2) else True,
                )
