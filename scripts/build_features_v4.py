#!/usr/bin/env python3
"""Build the V4 feature matrix for all seasons using the bulk pipeline.

Usage:
    poetry run python scripts/build_features_v4.py --seasons 2015-2025
    poetry run python scripts/build_features_v4.py --seasons 2024,2025
"""
...
from src.config import FEATURES_DIR, FEATURE_ORDER_V4
...
def main():
    parser = argparse.ArgumentParser(description="Build V4 features using bulk pipeline")
    ...
    print(f"Building V4 features for seasons: {seasons}")
    print(f"Output directory: {out_dir}")
    print(f"V4 features: {len(FEATURE_ORDER_V4)}")
    print()
    ...
    for season in seasons:
        ...
        try:
            ...
            # Check V4 features present
            present = [f for f in FEATURE_ORDER_V4 if f in df.columns]
            missing = [f for f in FEATURE_ORDER_V4 if f not in df.columns]

            # NaN analysis on V4 features
            feat_df = df[[f for f in FEATURE_ORDER_V4 if f in df.columns]]
            nan_rate = feat_df.isna().mean().mean() * 100
            complete_rows = (feat_df.isna().sum(axis=1) == 0).sum()

            print(f"  Games: {len(df)}")
            print(f"  V4 features: {len(present)}/{len(FEATURE_ORDER_V4)}")
            ...
            # Save full df (includes metadata columns + all V3+V4 features + scores)
            out_path = out_dir / f"season_{season}_v4_features.parquet"
    ...
    print("SUMMARY")
    print(f"{'='*60}")
    print(f"{'Season':>8} {'Games':>8} {'V4 Feats':>10} {'NaN%':>8} {'Complete':>10} {'Time':>8}")
    print("-" * 66)
    for r in results:
        if r["status"] == "ok":
